...
- Spark Structured Streaming 기반 Iceberg Bronze
...
첫째는 generator의 직접 SQL과 ORM 모델 사이의 계약 중복이다. 둘째는 `snapshot.mode=no_data` 상태에서 connector보다 먼저 생긴 outbox 이벤트를 복구하는 bootstrap 절차가 없다는 점이다. 셋째는 outbox 보존·정리 정책과 end-to-end reconciliation이 아직 없다는 점이다. P1에서는 Bronze 적재만 추가하기보다 이 실패 조건을 함께 검증해야 한다.
