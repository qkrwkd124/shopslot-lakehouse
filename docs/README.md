...
고정 P0의 MySQL/outbox/Kafka/Bronze 정합성과 Silver 정제·재실행 결과를 검증했다. 기준 데이터셋은 예약 10건, 이벤트 29건이며 추가 생성 실험 후의 건수와 구분한다. 지속 유입 중 장애 복구, 다중 노드 부하, Silver 증분 처리와 Gold 모델은 완료된 성과로 간주하지 않는다. 상세 결과와 당시 조건은 변경 기록을 참조한다.
